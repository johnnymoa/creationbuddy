...
function loadChat(index) {
    ...
    chatAreaElement.innerHTML = chats[index].messages.map((msg, msgIndex) => {
        ...
        if (isUser || isEdit) {
            ...
            return `
                <div class="message user" data-message-index="${msgIndex}">
                    <div class="message-bubble-wrapper">
                        <div class="message-content">${mainHTML}</div>
                        <div class="message-actions">
                            <button class="message-action-btn" onclick="copyMessage(${msgIndex})" title="Copy message">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" width="14" height="14">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M15.666 3.888A2.25 2.25 0 0013.5 2.25h-3c-1.03 0-1.9.693-2.166 1.638m7.332 0c.055.194.084.4.084.612v0a.75.75 0 01-.75.75H9a.75.75 0 01-.75-.75v0c0-.212.03-.418.084-.612m7.332 0c.646.049 1.288.11 1.927.184 1.1.128 1.907 1.077 1.907 2.185V19.5a2.25 2.25 0 01-2.25 2.25H6.75A2.25 2.25 0 014.5 19.5V6.257c0-1.108.806-2.057 1.907-2.185a48.208 48.208 0 011.927-.184" />
                                </svg>
                            </button>
                            <button class="message-action-btn" onclick="deleteFromMessage(${msgIndex})" title="Delete from here">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" width="14" height="14">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
                                </svg>
                            </button>
                            <button class="message-action-btn" onclick="repromptMessage(${msgIndex})" title="Edit and reprompt">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" width="14" height="14">
            ...
            if (thinkingHTML) {
                ...
                return `
                    <div class="message assistant" data-message-index="${msgIndex}">
                        ...
                        <div class="message-bubble-wrapper">
                            ...
                            <div class="message-actions assistant-actions">
                                <button class="message-action-btn" onclick="copyMessage(${msgIndex})" title="Copy response">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" width="14" height="14">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M15.666 3.888A2.25 2.25 0 0013.5 2.25h-3c-1.03 0-1.9.693-2.166 1.638m7.332 0c.055.194.084.4.084.612v0a.75.75 0 01-.75.75H9a.75.75 0 01-.75-.75v0c0-.212.03-.418.084-.612m7.332 0c.646.049 1.288.11 1.927.184 1.1.128 1.907 1.077 1.907 2.185V19.5a2.25 2.25 0 01-2.25 2.25H6.75A2.25 2.25 0 014.5 19.5V6.257c0-1.108.806-2.057 1.907-2.185a48.208 48.208 0 011.927-.184" />
                                    </svg>
                                </button>
                                <button class="message-action-btn" onclick="regenerateResponse(${msgIndex})" title="Regenerate response">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" width="14" height="14">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182m-4.991 4.99a.75.75 0 11-1.061-1.061.75.75 0 011.06 1.06z" />
                                    </svg>
                                </button>
                            </div>
                        </div>
...
async function deleteFromMessage(messageIndex) {
    const messagesCount = chats[currentChatIndex].messages.length;
    const deletedCount = messagesCount - messageIndex;
    
    const confirmed = await createConfirmationModal(
        'Delete Messages',
        `Are you sure you want to delete ${deletedCount} message${deletedCount > 1 ? 's' : ''} from this point onwards? This action cannot be undone.`,
        'Delete',
        'Cancel',
        true
    );
    ...
    if (confirmed) {
        ...
        chats[currentChatIndex].messages = chats[currentChatIndex].messages.slice(0, messageIndex);
        saveChats();
        loadChat(currentChatIndex);
...
function repromptMessage(messageIndex) {
    const message = chats[currentChatIndex].messages[messageIndex];
    if (!message) return;
    ...
    saveButton.className = 'message-edit-btn primary';
    saveButton.textContent = 'Save & Reprompt';
    saveButton.onclick = () => saveReprompt(messageIndex, textarea.value);
    
    const cancelButton = document.createElement('button');
    cancelButton.className = 'message-edit-btn secondary';
    cancelButton.textContent = 'Cancel';
    cancelButton.onclick = () => cancelReprompt(messageIndex);
    ...
    textarea.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') {
            cancelReprompt(messageIndex);
        } else if (e.key === 'Enter' && (e.ctrlKey || e.metaKey)) {
            e.preventDefault();
            saveReprompt(messageIndex, textarea.value);
        }
    });
}

async function saveReprompt(messageIndex, newContent) {
    if (!newContent.trim()) {
    ...
    if (awaitingResponse) {
        const confirmed = await createConfirmationModal(
            'Generation in Progress',
            'AI is currently generating a response. Are you sure you want to stop and reprompt?',
            'Stop & Reprompt',
            'Continue',
            false
        );
        ...
        if (confirmed) {
            ...
            return; // User chose to continue with current generation
        }
    }
    
    // Update the message content
    chats[currentChatIndex].messages[messageIndex].content = newContent.trim();
    
    // Remove all messages after this one (since we're reprompting)
    chats[currentChatIndex].messages = chats[currentChatIndex].messages.slice(0, messageIndex + 1);
    ...
    // Convert 'user_edit' role to 'user' for the API call
    messagesToSend.forEach(message => {
        if (message.role === 'user_edit') {
            message.role = 'user';
        }
...
function cancelReprompt(messageIndex) {
    const messageElement = document.querySelector(`[data-message-index="${messageIndex}"]`);
    if (!messageElement) return;
...
// Add regenerate function after the existing message action functions
async function regenerateResponse(messageIndex) {
    // Check if we're currently generating a response
    if (awaitingResponse) {
        const confirmed = await createConfirmationModal(
            'Generation in Progress',
            'AI is currently generating a response. Are you sure you want to stop and regenerate?',
            'Stop & Regenerate',
            'Continue',
            false
        );
    ...
    for (let i = messageIndex - 1; i >= 0; i--) {
        if (chats[currentChatIndex].messages[i].role === 'user' || chats[currentChatIndex].messages[i].role === 'user_edit') {
    ...
    // Show thinking indicator and generate new response
    awaitingResponse = true;
    sendButton.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="18" height="18"><path d="M6 6h12v12H6z"/></svg>`;
    sendButton.title = 'Stop generating';
    sendButton.classList.add('stop-button');
